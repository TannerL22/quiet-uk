...
def compare_places(pilot, places, release_id):
    ...
    for source in pilot.manifest['products']:
        ...
        for metric in pilot.manifest['metrics']:
            ...
            for o in observations[1:]:
                delta, same_cell = None, None
                if base['status'] != 'reported_model_value' or o['status'] != 'reported_model_value':
                ...
                differences.append({'difference_from_first_db': delta, 'status': status, 'same_native_cell': same_cell})
    ...
    return {'schema_version': 1, 'release_id': release_id, 'places': locations, 'rows': rows,
            ...
            'products': pilot.manifest['products'], 'reporting_cutoff_db': pilot.manifest['reporting_cutoff_db'],
            'aircraft_cutoff_note': pilot.manifest['aircraft_cutoff_note'],
...
def comparison_csv(result):
    ...
    for index, place in enumerate(result['places']):
        ...
        for row in result['rows']:
            ...
            writer.writerow({
                **{k: result[k] for k in ('release_id', 'spatial_resolution_m', 'receiver_height_m', 'missing_value_policy', 'uncertainty', 'aircraft_cutoff_note', 'licence_url', 'attribution')},
                ...
                **{k: o[k] for k in ('source', 'metric', 'value_db', 'raw_value', 'status', 'units', 'reference_period_status', 'coverage_id', 'source_sha256', 'http_record', 'row', 'column')},
                ...
                'reporting_cutoff_db': result['reporting_cutoff_db'][o['metric']],

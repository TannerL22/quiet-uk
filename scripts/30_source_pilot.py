...
from quiet_uk.source_pilot import SourcePilot, publish, publish_interpretation, SITES, acquisition_plan, compare_reference
...
def main():
    ...
    parser.add_argument('--reinterpret', type=Path, help='Publish new evidence semantics from this parent release, without network acquisition')
    ...
    if args.reinterpret and (args.verify or args.plan):
        parser.error('--reinterpret cannot be combined with --verify or --plan')
    ...
    if args.plan:
        ...
        return
    if not args.verify:
        manifest = publish_interpretation(args.reinterpret, args.output) if args.reinterpret else publish(args.output, sites=sites)
        print('Published', manifest['release_id'])
